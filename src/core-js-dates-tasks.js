...
function getNextFriday(date) {
  ...
  if (weekDay >= 5) {
    res.setDate(res.getDate() + 7 - (weekDay - 5));
  } else {
    res.setDate(res.getDate() + 5 - weekDay);
  }
  return res;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysInMonth; i += 1) {
    const curDay = new Date(year, month - 1, i).getDay();
...
function getWeekNumberByDate(date) {
  ...
  if (weekDay <= 4) {
    ...
    firstMonday.setDate(firstDay.getDate() + (7 - weekDay) + 1);
  }
  const deltaS = date - firstMonday;
  const deltaD = deltaS / (24 * 60 * 60 * 1000);
  return Math.ceil((deltaD + 1) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let workCycle = true;
  let daysLeft = countWorkDays;

  while (curDate <= endDate) {
    if (workCycle) {
      schedule.push(format(curDate));
    }

    daysLeft -= 1;
    if (daysLeft === 0) {
      workCycle = !workCycle;
      daysLeft = workCycle ? countWorkDays : countOffDays;
    }
